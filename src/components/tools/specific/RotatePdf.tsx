...
export function RotatePdf() {
    const toolData = getToolBySlug("rotate-pdf");
        ...
        <div className="flex-1 flex flex-col items-center justify-center py-20 px-4 relative min-h-[80vh] w-full">
            ...
            <div className="relative z-10 w-full text-center flex flex-col items-center max-w-4xl mx-auto">
                ...
                {status === "idle" && (
                     <div 
                        {...getRootProps()} 
                        className={`w-full max-w-2xl border-2 border-dashed rounded-3xl p-12 transition-all cursor-pointer flex flex-col items-center justify-center bg-zinc-50/50 dark:bg-zinc-900/50 ${
                            isDragActive ? "border-red-500 bg-red-500/5" : "border-zinc-300 dark:border-zinc-700 hover:border-zinc-400 dark:hover:border-zinc-500 hover:bg-zinc-100/50 dark:hover:bg-white/50 dark:bg-zinc-800/50"
                        }`}
                    >
                        <input {...getInputProps()} />
                ...
                {status === "options" && file && (
                    <div className="w-full max-w-2xl p-10 rounded-3xl border border-zinc-200 dark:border-zinc-700 bg-white/50 dark:bg-zinc-800/50 backdrop-blur-sm shadow-2xl flex flex-col gap-8">
                        ...
                        <div className="flex flex-col items-center gap-6 py-8">
                            <h3 className="text-xl font-bold text-white">Rotation applied to all pages: {rotationAngle}°</h3>
                            
                            <div className="flex gap-6">
                                <button 
                                    onClick={rotateLeft}
                                    className="p-6 rounded-2xl border border-zinc-200 dark:border-zinc-700 bg-zinc-800/80 hover:bg-zinc-700 hover:border-zinc-500 transition-all flex flex-col items-center gap-3 group"
                                >
                                    <RotateCcw className="w-10 h-10 text-zinc-400 group-hover:text-white transition-colors" />
                                    <span className="text-zinc-600 dark:text-zinc-400 font-medium group-hover:text-white">Rotate Left</span>
                                </button>
                                ...
                                <button 
                                    ...
                                    className="p-6 rounded-2xl border border-zinc-200 dark:border-zinc-700 bg-zinc-800/80 hover:bg-zinc-700 hover:border-zinc-500 transition-all flex flex-col items-center gap-3 group"
                                >
                                    <RotateCw className="w-10 h-10 text-zinc-400 group-hover:text-white transition-colors" />
                                    <span className="text-zinc-600 dark:text-zinc-400 font-medium group-hover:text-white">Rotate Right</span>
                                </button>
                            </div>
                        </div>
                        ...
                        <div className="flex gap-4">
                            ...
                            <button onClick={handleRotate} className="flex-1 px-8 py-4 rounded-xl bg-red-600 text-zinc-900 dark:text-white font-bold hover:bg-red-500 transition-all shadow-[0_0_20px_rgba(220,38,38,0.3)] hover:shadow-[0_0_30px_rgba(220,38,38,0.5)]">
                                Rotate PDF
                            </button>
                        </div>
                    </div>
                ...
                {status === "processing" && (
                     <div className="w-full max-w-2xl p-12 rounded-3xl border border-zinc-200 dark:border-zinc-700 bg-white/50 dark:bg-zinc-800/50 backdrop-blur-sm flex flex-col items-center gap-6 shadow-2xl">
                        ...
                        <h3 className="text-3xl font-bold text-zinc-900 dark:text-white mt-4">Rotating PDF...</h3>
                ...
                {status === "done" && resultUrl && (
                    <div className="w-full max-w-2xl mt-8 p-12 rounded-3xl border border-zinc-200 dark:border-zinc-700 bg-white/50 dark:bg-zinc-800/50 backdrop-blur-sm flex flex-col items-center gap-8 shadow-2xl">
                        ...
                        <div className="text-center">
                            <h3 className="text-3xl font-bold text-zinc-900 dark:text-white mb-3">PDF Rotated Successfully!</h3>
                            <p className="text-zinc-600 dark:text-zinc-400 text-lg">Your document has been updated.</p>
                        </div>
