...
import { Loader2, Download, CheckCircle2, FilePlus, X, Table } from "lucide-react";
import * as XLSX from "xlsx";
import jsPDF from "jspdf";

export function ExcelToPdf() {
    const toolData = getToolBySlug("excel-to-pdf");
    ...
    const handleConvert = async () => {
        ...
        try {
            ...
            workbook.SheetNames.forEach((sheetName, index) => {
                ...
                doc.setFontSize(16);
                doc.text(`Sheet: ${sheetName}`, 14, currentY);
                currentY += 10;
                doc.setFontSize(10);

                json.slice(0, 50).forEach((row) => { // Limit to 50 rows for simplicity
                    if (currentY > 280) {
                        doc.addPage();
                        currentY = 20;
                    }
                    const rowText = row.join(" | ").substring(0, 100);
                    doc.text(rowText, 14, currentY);
                    currentY += 7;
                });
            });
            ...
            setError("Conversion failed. Ensure it's a valid Excel file.");
            setStatus("options");
        }
    };
            ...
            <div className="relative z-10 w-full text-center flex flex-col items-center max-w-4xl mx-auto">
                ...
                {status === "idle" && (
                    ...
                    >
                        ...
                        <p className="text-xl font-bold text-zinc-900 dark:text-white mb-2">
                            {isDragActive ? "Drop Spreadsheet here" : "Click or drag your Excel file here"}
                        </p>
                        <p className="text-zinc-500 dark:text-zinc-500">
                            Upload a .xlsx or .xls file to convert to PDF
                        </p>
                    </div>
                )}
                ...
                {status === "options" && file && (
                    <div className="w-full max-w-2xl p-8 rounded-3xl border border-zinc-200 dark:border-zinc-700 bg-white/50 dark:bg-zinc-800/50 backdrop-blur-sm shadow-2xl flex flex-col gap-8">
                        ...
                        <div className="text-left w-full bg-green-900/20 border border-green-500/30 p-6 rounded-2xl">
                            <h3 className="text-xl font-bold text-green-400 mb-2 flex items-center gap-3">
                                <Table className="w-5 h-5" />
                                Ready for Table Conversion
                            </h3>
                            <p className="text-green-300/70 text-sm">
                                We will parse your spreadsheet cells and generate a clean PDF table layout for all active sheets.
                            </p>
                        </div>
                ...
                {status === "processing" && (
                    <div className="w-full max-w-2xl p-12 rounded-3xl border border-zinc-200 dark:border-zinc-700 bg-white/50 dark:bg-zinc-800/50 backdrop-blur-sm flex flex-col items-center gap-6 shadow-2xl">
                        ...
                        <h3 className="text-3xl font-bold text-zinc-900 dark:text-white mt-4">Parsing Sheets...</h3>
                        <p className="text-zinc-600 dark:text-zinc-400 text-lg">Converting workbook data to PDF pages.</p>
                    </div>
                )}
                ...
                {status === "done" && resultUrl && (
                    <div className="w-full max-w-2xl mt-8 p-12 rounded-3xl border border-zinc-200 dark:border-zinc-700 bg-white/50 dark:bg-zinc-800/50 backdrop-blur-sm flex flex-col items-center gap-8 shadow-2xl">
                        ...
                        <div className="text-center">
                            ...
                            <p className="text-zinc-600 dark:text-zinc-400 text-lg">Your spreadsheet is now a PDF document.</p>
                        </div>
                        ...
                        <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto mt-4">
                            ...
                            <a
                                href={resultUrl}
                                download={`${file?.name.replace('.xlsx', '').replace('.xls', '') || "spreadsheet"}.pdf`}
