            ...
            <div className="relative z-10 w-full text-center flex flex-col items-center max-w-4xl mx-auto">
                ...
                {status === "idle" && (
                    <div className="w-full max-w-2xl flex flex-col gap-6">
                        <div 
                            {...getRootProps()} 
                            className={`border-2 border-dashed rounded-3xl p-12 transition-all cursor-pointer flex flex-col items-center justify-center bg-zinc-50/50 dark:bg-zinc-900/50 ${
                        ...
                        {files.length > 0 && (
                            <div className="flex flex-col gap-3 text-left w-full mt-4">
                                ...
                                <div className="flex flex-col gap-2">
                                    {files.map((file, i) => (
                                        <div key={`${file.name}-${i}`} className="flex items-center gap-3 bg-white/50 dark:bg-zinc-800/50 border border-zinc-200 dark:border-zinc-700 p-3 rounded-xl group transition-all hover:bg-zinc-800">
                                            <div className="flex flex-col gap-1 items-center opacity-0 group-hover:opacity-100 transition-opacity">
                                                <button onClick={() => moveFile(i, 'up')} disabled={i === 0} className="text-zinc-500 dark:text-zinc-500 hover:text-white disabled:opacity-30">▲</button>
                                                <button onClick={() => moveFile(i, 'down')} disabled={i === files.length - 1} className="text-zinc-500 dark:text-zinc-500 hover:text-white disabled:opacity-30">▼</button>
                                            </div>
                                            <GripVertical className="w-5 h-5 text-zinc-500 dark:text-zinc-500 cursor-grab active:cursor-grabbing" />
                                            <span className="text-zinc-300 font-medium flex-1 truncate">{file.name}</span>
                                            <span className="text-zinc-500 dark:text-zinc-500 text-sm">{(file.size / 1024 / 1024).toFixed(2)} MB</span>
                                            <button onClick={() => removeFile(i)} className="p-2 text-zinc-500 dark:text-zinc-500 hover:text-red-500 hover:bg-red-500/10 rounded-lg transition-all">
                                                <X className="w-5 h-5" />
                                            </button>
                                        </div>
                                    ))}
                ...
                {status === "processing" && (
                    <div className="w-full max-w-2xl mt-8 p-12 rounded-3xl border border-zinc-200 dark:border-zinc-700 bg-white/50 dark:bg-zinc-800/50 backdrop-blur-sm flex flex-col items-center gap-6 shadow-2xl">
                        ...
                        <h3 className="text-3xl font-bold text-zinc-900 dark:text-white mt-4">Merging PDFs...</h3>
                        <p className="text-zinc-600 dark:text-zinc-400 text-lg">Combining {files.length} files securely in your browser.</p>
                    </div>
                )}
                ...
                {status === "done" && mergedPdfUrl && (
                    <div className="w-full max-w-2xl mt-8 p-12 rounded-3xl border border-zinc-200 dark:border-zinc-700 bg-white/50 dark:bg-zinc-800/50 backdrop-blur-sm flex flex-col items-center gap-8 shadow-2xl">
                        ...
                        <div className="text-center">
                            <h3 className="text-3xl font-bold text-zinc-900 dark:text-white mb-3">PDFs Merged Successfully!</h3>
                            <p className="text-zinc-600 dark:text-zinc-400 text-lg">Your files have been combined into a single document.</p>
                        </div>
