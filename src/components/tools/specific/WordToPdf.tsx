...
export function WordToPdf() {
    ...
    const handleConvert = async () => {
        ...
        try {
            ...
            // Mammoth only supports .docx
            if (file.name.toLowerCase().endsWith('.doc')) {
                throw new Error("Standard .doc files are not supported client-side. Please use .docx.");
            }

            const { value: html } = await mammoth.convertToHtml({ arrayBuffer });

            // Create PDF from text (simplified conversion)
            const doc = new jsPDF();
            const textLines = doc.splitTextToSize(html.replace(/<[^>]*>/g, ' '), 180);
            ...
            setError(err.message || "Conversion failed. Ensure it's a valid .docx file.");
            setStatus("options");
        }
    };
            ...
            <div className="relative z-10 w-full text-center flex flex-col items-center max-w-4xl mx-auto">
                ...
                {status === "idle" && (
                    ...
                    >
                        ...
                        <p className="text-xl font-bold text-zinc-900 dark:text-white mb-2">
                            {isDragActive ? "Drop Word doc here" : "Click or drag your Word file here"}
                        </p>
                    ...
                    <div className="w-full max-w-2xl p-8 rounded-3xl border border-zinc-200 dark:border-zinc-700 bg-white/50 dark:bg-zinc-800/50 backdrop-blur-sm shadow-2xl flex flex-col gap-8">
                        ...
                        <div className="text-left w-full bg-blue-900/20 border border-blue-500/30 p-6 rounded-2xl">
                            ...
                            <p className="text-blue-300/70 text-sm">
                                We will extract the text and basic formatting from your Word document to generate a high-quality PDF.
                            </p>
                        </div>
                ...
                {status === "processing" && (
                    <div className="w-full max-w-2xl p-12 rounded-3xl border border-zinc-200 dark:border-zinc-700 bg-white/50 dark:bg-zinc-800/50 backdrop-blur-sm flex flex-col items-center gap-6 shadow-2xl">
                        ...
                        <p className="text-zinc-600 dark:text-zinc-400 text-lg">Processing Word XML structures.</p>
                    </div>
                )}
                ...
                {status === "done" && resultUrl && (
                    <div className="w-full max-w-2xl mt-8 p-12 rounded-3xl border border-zinc-200 dark:border-zinc-700 bg-white/50 dark:bg-zinc-800/50 backdrop-blur-sm flex flex-col items-center gap-8 shadow-2xl">
                        ...
                        <div className="text-center">
                            ...
                            <p className="text-zinc-600 dark:text-zinc-400 text-lg">Your PDF has been generated successfully.</p>
                        </div>
                        ...
                        <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto mt-4">
                            ...
                            <a
                                href={resultUrl}
                                download={`${file?.name.replace('.docx', '') || "document"}.pdf`}
