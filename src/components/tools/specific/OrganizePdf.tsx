    ...
    return (
        <div className="flex-1 flex flex-col items-center justify-center py-20 px-4 relative min-h-[80vh] w-full">
            ...
            <div className="relative z-10 w-full text-center flex flex-col items-center max-w-4xl mx-auto">
                ...
                {status === "options" && file && (
                    <div className="w-full max-w-2xl p-8 md:p-10 rounded-3xl border border-zinc-700 bg-zinc-800/50 backdrop-blur-sm shadow-2xl flex flex-col">
                        <div className="flex items-center justify-between pb-6 border-b border-zinc-700 mb-8">
                            <div className="flex items-center gap-4 text-left overflow-hidden mr-4">
                                ...
                                <div className="min-w-0">
                                    <p className="text-white font-bold text-lg truncate w-full">{file.name}</p>
                                    <p className="text-zinc-400 text-sm">{(file.size / 1024 / 1024).toFixed(2)} MB • {pagesState.length} pages</p>
                                </div>
                            </div>
                            <button onClick={handleReset} className="p-2 shrink-0 text-zinc-500 hover:text-red-500 hover:bg-red-500/10 rounded-lg transition-all">
                                <X className="w-6 h-6" />
                            </button>
                        </div>
                            ...
                            {pagesState.length === 0 ? (
                                ...
                                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                                    {pagesState.map((page, index) => (
                                        <div key={page.id} className="bg-zinc-900 border border-zinc-700 rounded-xl p-4 flex flex-col gap-3 group hover:border-zinc-500 transition-colors">
                                            <div className="flex justify-between items-start">
                                                <span className="text-sm font-bold text-zinc-500 group-hover:text-zinc-300">New Page {index + 1}</span>
                                            ...
                                            <div className="flex-1 flex flex-col items-center justify-center py-6 bg-zinc-800 rounded-lg border border-zinc-700">
                        ...
                        <div className="flex gap-4">
                            ...
                            <button
                                ...
                                className="flex-1 px-8 py-4 rounded-xl bg-red-600 text-white font-bold hover:bg-red-500 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-[0_0_20px_rgba(220,38,38,0.3)] hover:shadow-[0_0_30px_rgba(220,38,38,0.5)]"
                            >
                                Generate PDF
                            </button>
                        </div>
                ...
                {status === "processing" && (
                    <div className="w-full max-w-2xl p-12 rounded-3xl border border-zinc-700 bg-zinc-800/50 backdrop-blur-sm flex flex-col items-center gap-6 shadow-2xl">
                        ...
                        <h3 className="text-3xl font-bold text-white mt-4">Organizing PDF...</h3>
                ...
                {status === "done" && resultUrl && (
                    <div className="w-full max-w-2xl mt-8 p-12 rounded-3xl border border-zinc-700 bg-zinc-800/50 backdrop-blur-sm flex flex-col items-center gap-8 shadow-2xl">
                        ...
                        <div className="text-center">
                            <h3 className="text-3xl font-bold text-white mb-3">PDF Organized!</h3>
                            <p className="text-zinc-400 text-lg">Your document has been successfully updated.</p>
                        </div>
