...
import { Loader2, Download, CheckCircle2, FilePlus, X, Presentation } from "lucide-react";
import JSZip from "jszip";
import jsPDF from "jspdf";

export function PptToPdf() {
    const toolData = getToolBySlug("ppt-to-pdf");
    ...
    const handleConvert = async () => {
        ...
        try {
            ...
            // Extract text from slides (ppt/slides/slideN.xml)
            ...
            for (const slidePath of slideFiles.sort()) {
                ...
                // Simple regex to find text content in ppt xml
                ...
                if (matches) {
                    ...
                    doc.setFontSize(16);
                    doc.text(`Slide ${pageCount}`, 10, 20);
                    doc.setFontSize(10);
            ...
            console.error("Failed to convert PPT to PDF", err);
            setError("Conversion failed. Ensure it's a valid .pptx file.");
            setStatus("options");
        }
    };
            ...
            <div className="relative z-10 w-full text-center flex flex-col items-center max-w-4xl mx-auto">
                ...
                {status === "idle" && (
                    ...
                    >
                        ...
                        <p className="text-xl font-bold text-zinc-900 dark:text-white mb-2">
                            {isDragActive ? "Drop Presentation here" : "Click or drag your PPTX file here"}
                        </p>
                        <p className="text-zinc-500 dark:text-zinc-500">
                            Upload a .pptx file to convert to PDF (text-based)
                        </p>
                    </div>
                )}
                ...
                {status === "options" && file && (
                    <div className="w-full max-w-2xl p-8 rounded-3xl border border-zinc-200 dark:border-zinc-700 bg-white/50 dark:bg-zinc-800/50 backdrop-blur-sm shadow-2xl flex flex-col gap-8">
                        ...
                        <div className="text-left w-full bg-orange-900/20 border border-orange-500/30 p-6 rounded-2xl">
                            <h3 className="text-xl font-bold text-orange-400 mb-2 flex items-center gap-3">
                                <Presentation className="w-5 h-5" />
                                Ready for Presentation Scan
                            </h3>
                            <p className="text-orange-300/70 text-sm">
                                We will extract text content slide-by-slide and generate a structured PDF document. Note: Images and complex formatting are not preserved in this version.
                            </p>
                        </div>
                ...
                {status === "processing" && (
                    <div className="w-full max-w-2xl p-12 rounded-3xl border border-zinc-200 dark:border-zinc-700 bg-white/50 dark:bg-zinc-800/50 backdrop-blur-sm flex flex-col items-center gap-6 shadow-2xl">
                        ...
                        <h3 className="text-3xl font-bold text-zinc-900 dark:text-white mt-4">Analyzing Slides...</h3>
                        <p className="text-zinc-600 dark:text-zinc-400 text-lg">Decompressing presentation package.</p>
                    </div>
                )}
                ...
                {status === "done" && resultUrl && (
                    <div className="w-full max-w-2xl mt-8 p-12 rounded-3xl border border-zinc-200 dark:border-zinc-700 bg-white/50 dark:bg-zinc-800/50 backdrop-blur-sm flex flex-col items-center gap-8 shadow-2xl">
                        ...
                        <div className="text-center">
                            ...
                            <p className="text-zinc-600 dark:text-zinc-400 text-lg">Slides have been converted to a textual PDF.</p>
                        </div>
                        ...
                        <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto mt-4">
                            ...
                            <a
                                href={resultUrl}
                                download={`${file?.name.replace('.pptx', '') || "presentation"}.pdf`}
