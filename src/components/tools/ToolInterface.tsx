    ...
    return (
        <div className="flex-1 flex flex-col items-center justify-center py-20 px-4 relative min-h-[80vh] w-full">
            ...
            <div className="relative z-10 w-full text-center flex flex-col items-center max-w-4xl mx-auto">
                ...
                {fileStatus === "selected" && (
                    <div className="w-full max-w-2xl mt-8 p-10 rounded-3xl border border-zinc-700 bg-zinc-800/50 backdrop-blur-sm flex flex-col items-center gap-8 shadow-2xl">
                        <div className="flex flex-col items-center gap-4 text-zinc-300">
                            <Icon className="w-12 h-12 text-red-500" />
                            <span className="text-xl font-medium truncate max-w-[300px]">{fileName}</span>
                            <span className="text-sm text-zinc-500">Ready for processing</span>
                        </div>

                        <div className="flex gap-4 w-full sm:w-auto">
                            <button onClick={handleReset} className="flex-1 sm:flex-none px-6 py-4 rounded-xl border border-zinc-600 text-zinc-300 font-medium hover:bg-zinc-700/50 hover:text-white transition-all cursor-pointer">
                                Cancel
                            </button>
                ...
                {fileStatus === "processing" && (
                    <div className="w-full max-w-2xl mt-8 p-12 rounded-3xl border border-zinc-700 bg-zinc-800/50 backdrop-blur-sm flex flex-col items-center gap-6 shadow-2xl">
                        ...
                        <h3 className="text-3xl font-bold text-white mt-4">Processing...</h3>
                        <p className="text-zinc-400 text-lg">Please wait while we perform the operation.</p>
                ...
                {fileStatus === "done" && (
                    <div className="w-full max-w-2xl mt-8 p-12 rounded-3xl border border-zinc-700 bg-zinc-800/50 backdrop-blur-sm flex flex-col items-center gap-8 shadow-2xl">
                        ...
                        <div className="text-center">
                            <h3 className="text-3xl font-bold text-white mb-3">Task Complete!</h3>
                            <p className="text-zinc-400 text-lg">Your file has been processed successfully.</p>
                        </div>

                        <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto mt-4">
                            <button onClick={handleReset} className="px-6 py-4 rounded-xl border border-zinc-600 text-zinc-300 font-medium hover:bg-zinc-700/50 hover:text-white transition-all cursor-pointer">
                                Process Another
                            </button>
